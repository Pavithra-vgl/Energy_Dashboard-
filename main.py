...
def latest(db: Session = Depends(get_db)):
    row = (
        db.query(TimeSeriesPoint.ts)
        .filter(TimeSeriesPoint.metric == "load", TimeSeriesPoint.resolution == "hour", TimeSeriesPoint.region == "DE")
        .order_by(TimeSeriesPoint.ts.desc())
        .first()
    )
    return {"latest_ts": row[0] if row else None}
